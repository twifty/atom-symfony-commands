...
export default class SymfonyCommandDetailView
{
        ...
        etch.getScheduler().updateDocument(async () => {
            try {
                ...
                if (!this.commands.length) {
                    await this.refs.commandListView.update({
                        loadingMessage: null, errorMessage: <span className="error">{"No commands are available!"}</span>
                    })
                } else {
                    await this.refs.commandListView.update({items: this.commands, loadingMessage: null})
                }
            } catch (error) {
                console.error(error)
            }
    ...
    onConfirm (action) {
        ...
        if (command) {
            this.show(false)
            this.emitter.emit(action, {command})
        }
    }
