...
export default class ConsoleView
{
  ...
  constructor (projectManager, config) {
    ...
    this.disposables.add(this.projectManager.observeProjects(projects => {
      ...
      'symfony-console:copy': () => {atom.clipboard.write(this.refs.terminal.getSelection())},
			'symfony-console:paste': () => {this.refs.terminal.pasteContent(atom.clipboard.read())}, // This should write to text input
			'symfony-console:clear': () => {this.refs.terminal.clear()},
      'symfony-console:reset': () => {this.refs.terminal.clear()},
			'symfony-console:interrupt': () => {sendSignal('signal', 'SIGINT')},
			'symfony-console:terminate': () => {sendSignal('signal', 'SIGTERM')},
			'symfony-console:kill': () => {sendSignal('signal', 'SIGKILL')},
		}))
  ...
  onUpdateComposer () {
    ...
    if (project) {
      const options = {
        onCmdLine: (data) => {this.refs.terminal.writeln(data + '\n')},
        onOutData: (data) => {this.refs.terminal.write(data)},
        onErrData: (data) => {this.refs.terminal.write(data)},
      }
    ...
    if (command) {
      ...
      if (project) {
        const options = {
          ...
          onOutData: (data) => {this.refs.terminal.write(data)},
          onErrData: (data) => {this.refs.terminal.write(data)},
          writeData: (stream) => {this.refs.terminal.on('input', stream)},
          sendSignal: (stream) => {this.refs.terminal.on('signal', stream)}
        }
