...
export default class ConsoleView {
  ...
  onEnterCommand (command, project) {
    ...
    if (command) {
      ...
      let {path, cwd} = this.consoleProperties[this.selectedProject]

      this.terminal.reset()
      this.terminal.focus()

      SymfonyConsole.runCommand(`${path} ${command}`, cwd, this, {
  ...
  populateProjectChoices () {
    ...
    if (consoleKeys.length <= 1) {
      this.refs.selectedProject.style.display = 'none'
    } else {
      this.refs.selectedProject.style.display = ''
