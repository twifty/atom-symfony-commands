...
export default class SymfonyCommandEditorView
{
        ...
        if (this.hintEnabled) {
            ...
            if (project) {
                getHints = project.listCommands(true).then(hints => {
                    ...
                    if (hints.length) {
                        hints = this.query
                            ? fuzzaldrin.filter(hints, this.query, {maxResults: 5})
                            : []
                        if (hints.length === 1)
                            hints = []
    ...
    render () {
        ...
        this.hints.forEach((item, index) => {
            items.push(
                <SelectableItem item={item} selected={this.getSelectedHint() === item} query={this.query} onClick={() => {
                    this.updateHints({selected: index})
                }} onDoubleClick={() => {
